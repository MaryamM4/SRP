...
class ServoHandler(threading.Thread):
	...
	fitted_pan =  min(self.MAX_PAN, max(self.MIN_PAN, pan))
	fitted_tilt = min(self.MAX_TILT, max(self.MIN_TILT, tilt))

	print(f'pan={fitted_pan} tilt={fitted_tilt}')
   
	# Make sure have reasonable values
        self.goal_pan = fitted_pan
        self.goal_tilt = fitted_tilt
